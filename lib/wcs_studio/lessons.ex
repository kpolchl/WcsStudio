defmodule WcsStudio.Lesson do
  ...
  schema "lessons" do
    ...
    many_to_many :instructors, WcsStudio.Accounts.User, join_through: "lessons_instructors", join_keys: [lesson_id: :id, instructor_id: :id], on_replace: :delete
  ...
  def add(title, instructor_ids, pattern_ids, level_id, place, lesson_vid_url, date, dance_type_id) do
    instructors = WcsStudio.Repo.all(from u in WcsStudio.Accounts.User, where: u.id in ^instructor_ids)
    patterns = WcsStudio.Repo.all(from p in WcsStudio.Pattern, where: p.id in ^pattern_ids)

    %__MODULE__{}
    |> changeset(%{title: title, level_id: level_id, place: place, lesson_vid_url: lesson_vid_url, date: date, dance_type_id: dance_type_id})
  ...
  def update(lesson, title, instructor_ids, pattern_ids, level_id, place, lesson_vid_url, date, dance_type_id) do
    # Convert string IDs to integers if needed
    instructor_ids = Enum.map(instructor_ids, fn id ->
      if is_binary(id), do: String.to_integer(id), else: id
    end)
    pattern_ids = Enum.map(pattern_ids, fn id ->
      if is_binary(id), do: String.to_integer(id), else: id
    end)
